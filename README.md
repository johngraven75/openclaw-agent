...
OpenClaw is a local desktop-friendly multimodal agent workspace with a Flask backend and responsive Codex-inspired web UI.
...
- Conversational agent shell with local-first reasoning fallback.
- Chat task deck for web search, code execution, image generation, add-on lookup, workspace files, and credential access from the Chat screen.
...
- Hugging Face LLM chat through the current OpenAI-compatible Inference Providers router when the token is verified and usable.
- Hugging Face image generation adapter for text-to-image models when a token is configured.
- Provider settings for Hugging Face, OpenAI, Ollama, and OpenAI-compatible custom endpoints, including Hugging Face credential-state reporting.
...
If Hugging Face has a missing, invalid, rejected, unverified, or quota-blocked credential state, Chat remains in local reasoning mode and keeps the selected Hugging Face model dormant until the credential state is fixed.
