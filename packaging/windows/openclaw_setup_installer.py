...
def main() -> None:
    ...
    for name in ("OpenClaw.exe", "BUILD_NOTES_openclaw_build109.md", "README.md"):
        shutil.copy2(src / name, install_dir / name)

    for desktop in desktop_locations():
        launcher = desktop / "START OpenClaw Build 1.0.9.bat"
        launcher.write_text(
            '@echo off\nstart "" "%LOCALAPPDATA%\\Programs\\OpenClaw Agent\\OpenClaw.exe"\n',
            encoding="utf-8",
        )
        create_shortcut(install_dir / "OpenClaw.exe", desktop / "OpenClaw Build 1.0.9.lnk")
